...
methods
    ...
    function varargout = pack( s, varargin )
        switch nargin
            case 1
                % packet = s.pack();
                packetType = 'R0';
                type = s.typeConv(packetType) * 16;
                packet = uint8(['KS', type, 0, 0, 0, 0, 13]);
            ...
            case 2
                ...
                packetType = 'R0';
                type = s.typeConv(packetType) * 16;
                packet = uint8(['KS', type, 0, param(1), param(2), 0, 13]);
                packet(7) = s.getChecksum(packet(3:6)');
            case 3
    ...
    function varargout = packetSend( s, varargin )
        switch nargin
            ...
            case 2
                ...
                [packet, packetType] = s.pack(varargin{1});
            case 3
                % bytes = s.packetSend(param, type);
                % bytes = s.packetSend(param, data);
                [packet, packetType] = s.pack(varargin{1}, varargin{2});
            case 4
                % bytes = s.packetSend(param, data, type);
                [packet, packetType] = s.pack(varargin{1}, varargin{2}, varargin{3});
        end
    ...
    function [lens, type, data] = getTypesAndLens( s, data, typestr )
        ...
        type = s.typeConv(typestr) * 16 + fix(lens / 256);
        lens = mod(lens, 256);
        if i == 1
            data = data(:, 1)';
        elseif i == 2
